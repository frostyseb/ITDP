...
Class event extends Dbh{
    ...
    public function get_unjoin_event($uid){
        // SELECT * FROM events t1
        // join ref_event_types t2 ON t2.event_type_code = t1.event_type_code
        // join ref_event_status t3 ON t3.event_status_code = t1.event_status_code
        // WHERE t1.event_id not in (SELECT event_id FROM has_attend WHERE user_id = 3)
        $query = "SELECT * FROM events t1 join ref_event_types t2 ON t2.event_type_code = t1.event_type_code join ref_event_status t3 ON t3.event_status_code = t1.event_status_code WHERE t1.event_id not in (SELECT event_id FROM has_attend WHERE user_id = 3)
        ";
